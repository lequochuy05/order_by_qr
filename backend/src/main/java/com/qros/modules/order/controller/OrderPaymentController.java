...
public class OrderPaymentController {
    ...
    public ApiResponse<String> payOrder(
        ...
        return ApiResponse.success("Pay order successfully", orderService.payOrder(orderId, currentUserId, request));
    }

    @PostMapping("/{orderId}/confirm-paid")
    public ApiResponse<OrderResponse> confirmPaid(@PathVariable @NonNull Long orderId) {
        return ApiResponse.success("Confirm paid successfully", orderService.confirmPaid(orderId));
    }

    @PostMapping("/{orderId}/reconcile")
    public ApiResponse<OrderResponse> reconcileOrder(@PathVariable @NonNull Long orderId) {
        return ApiResponse.success("Reconcile successfully", orderService.reconcileOrder(orderId));
    }
}
