...
public class AnalyticsService {
        ...
        public DashboardSummaryResponse getDashboardSummary(LocalDate from, LocalDate to) {
                ...
                DashboardSummaryProjection summary = analyticsQueryRepository.dashboardSummary(
                                range.from(),
                                range.toExclusive());
                ...
                OrderSummaryProjection orderSummary = analyticsQueryRepository.orderSummary(
                                todayStart,
                                tomorrowStart);
        ...
        public List<RevenuePointResponse> getRevenueSeries(LocalDate from, LocalDate to) {
                ...
                return analyticsQueryRepository.revenueByDay(range.from(), range.toExclusive())
                                .stream()
        ...
        public Page<OrderDetailResponse> getOrderDetails(
                        LocalDate from,
                        LocalDate to,
                        Pageable pageable) {
                DateRange range = normalizeDateRange(from, to, DEFAULT_DAYS);

                return analyticsQueryRepository.orderDetails(
                                range.from(),
                                range.toExclusive(),
                                pageable)
        ...
        public List<SalesTrendPointResponse> getSalesTrend(LocalDate from, LocalDate to) {
                ...
                return analyticsQueryRepository.salesTrendByDay(range.from(), range.toExclusive())
                                .stream()
                                .map(row -> new SalesTrendPointResponse(
                                                LocalDate.parse(row.getDate()),
                                                safeLong(row.getQuantitySold())))
                                .toList();
        }
        ...
        private List<RecentOrderResponse> getRecentOrders(
                        ...
                        @NonNull LocalDateTime toExclusive,
                        int limit) {
                return analyticsQueryRepository.recentOrders(
                                from,
                                toExclusive,
                                sanitizeLimit(limit))
                                .stream()
