...
public class RateLimitingFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request,
        ...
        Deque<Long> timestamps = requestLogs.compute(clientIp, (key, existing) -> {
            Deque<Long> deque = (existing != null) ? existing : new ArrayDeque<>();

            // Prune entries outside the current window
            long cutoff = now - WINDOW_DURATION_MS;
            while (!deque.isEmpty() && deque.peekFirst() < cutoff) {
                deque.pollFirst();
            }

            // Check limit before adding this request
            if (deque.size() >= MAX_REQUESTS_PER_WINDOW) {
                return deque; // do NOT add the current timestamp — reject
            }
