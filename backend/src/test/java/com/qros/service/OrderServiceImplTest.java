...
class OrderServiceImplTest {
        ...
        void createOrderMergesMatchingUnpreparedItemForSameTable() {
                ...
                when(orderRepository.findFirstByTableIdAndStatusInForUpdate(10L,
                                java.util.List.of(Order.OrderStatus.PENDING, Order.OrderStatus.SERVING,
                ...
                org.mockito.Mockito.doAnswer(invocation -> {
                    ...
                    o.getTable().setStatus(DiningTable.TableStatus.OCCUPIED);
                    return null;
                }).when(orderStatusService).recalcTableStatus(any(Order.class));
        ...
        void orderStatsUsesNullSafeDefaultCacheKey() throws Exception {
                Cacheable cacheable = OrderQueryService.class
                                .getMethod("getOrderStats", String.class, java.time.LocalDate.class,
        ...
        void finishingLastKitchenItemPromotesOrderAndRecordsChangedBy() {
                ...
                when(orderItemRepository.saveAndFlush(any(OrderItem.class)))
                                .thenAnswer(invocation -> invocation.getArgument(0));
        ...
        void kitchenOrdersIncludeRecentlyFinishedItemsButExcludeOldFinishedOnlyOrders() {
                Order recentFinishedOrder = kitchenOrder(1L, OrderItem.OrderItemStatus.FINISHED,
                                AppTime.now().minusMinutes(5));
                Order oldFinishedOrder = kitchenOrder(2L, OrderItem.OrderItemStatus.FINISHED,
                                AppTime.now().minusMinutes(20));
                Order pendingOrder = kitchenOrder(3L, OrderItem.OrderItemStatus.PENDING,
                                AppTime.now().minusMinutes(30));

                when(orderRepository.findByStatusIn(List.of(Order.OrderStatus.PENDING, Order.OrderStatus.SERVING,
                                Order.OrderStatus.AWAITING_PAYMENT)))
