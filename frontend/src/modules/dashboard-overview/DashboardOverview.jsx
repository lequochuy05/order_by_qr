    ...
    useEffect(() => {
        ...
        const fetchDashboardData = async () => {
            ...
            try {
                ...
                setRevenueForecast(Array.isArray(dashboard.revenueForecast) ? dashboard.revenueForecast : []);
                setPopularDishesForecast(
                    Array.isArray(dashboard.popularDishesForecast)
                ...
                <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-100">
                    ...
                    <div className="flex flex-col gap-4">
                        {recentOrders.map((order, idx) => {
                            ...
                            return (
                                <div key={order.id || idx} className="flex justify-between items-start p-3 border-b border-slate-50 last:border-0 hover:bg-slate-50 rounded-xl transition-colors">
                                    <div>
                                        <p className="text-sm font-semibold text-slate-700">Đơn #{order.id}</p>
                                        <p className="text-xs text-slate-500">{tableNumber ? `Bàn ${tableNumber}` : 'Mang đi'} • {fmtTime(order.paymentTime || order.createdAt)}</p>
                                    </div>
