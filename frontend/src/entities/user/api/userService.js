...
export const userService = {
  getPage: async (params = {}, { force = false } = {}) => {
    ...
    const requestParams = { page, size, sort: 'fullName,asc', ...params };
    ...
    if (!userRequests.has(key)) {
      const request = api
        ...
        .then((res) => {
          userCache.set(key, { data: res, expiresAt: Date.now() + STAFF_LIST_CACHE_MS });
          return res;
        })
  ...
  create: async (data) => { clearUserCache(); const res = await api.post('/users', data); clearUserCache(); return res; },
  update: async (id, data) => { clearUserCache(); const res = await api.put(`/users/${id}`, data); clearUserCache(); return res; },
  ...
  delete: async (id) => { clearUserCache(); await api.delete(`/users/${id}`); clearUserCache(); },
};
