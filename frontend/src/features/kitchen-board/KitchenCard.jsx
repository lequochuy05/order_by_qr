...
const KitchenCard = ({ item, status, onUpdateStatus, isProcessing }) => {
  ...
  return (
    <article
      className={`overflow-hidden rounded-2xl border bg-white shadow-sm transition hover:-translate-y-0.5 hover:shadow-md dark:bg-slate-900 ${priority.card}`}
    >
...
const WaitBadge = ({ item, priority }) => {
  ...
  return (
    <span
      className={`inline-flex items-center gap-1 rounded-lg px-2 py-1 text-xs font-black ${priority.badge}`}
    >
...
const ActionButton = ({ onClick, disabled, loading, icon, label, loadingLabel, className }) => (
  <button
    type="button"
    onClick={onClick}
    disabled={disabled}
