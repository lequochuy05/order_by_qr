    ...
    <div className="fixed inset-0 bg-black/60 flex items-end z-50 animate-in fade-in duration-300">
      <div className="bg-white dark:bg-slate-900 w-full max-w-md mx-auto rounded-t-[2rem] p-6 max-h-[85vh] flex flex-col shadow-2xl animate-in slide-in-from-bottom duration-500 transition-colors">
        ...
        <div className="flex justify-between items-center mb-6">
          ...
          <button
            onClick={onClose}
            className="p-2 bg-gray-100 dark:bg-slate-800 rounded-full hover:bg-gray-200 dark:hover:bg-slate-700 text-gray-600 dark:text-gray-300 transition-colors"
          >
            <X size={20} />
          </button>
        </div>
        ...
        <div className="flex-1 overflow-y-auto pr-2 space-y-4">
          ...
          {Object.entries(cart.items || {}).map(([id, item], index) => (
            <div
              key={id}
              className="border-b border-stone-200/70 dark:border-slate-800 pb-4 transition-colors animate-stagger-fade"
              style={{ animationDelay: `${index * 30}ms` }}
            >
              <div className="flex justify-between items-start font-bold text-sm text-gray-800 dark:text-white transition-colors">
                <div className="flex flex-col flex-1">
                  <span>
                    {item.name} <span className="text-orange-500 ml-1">x{item.qty}</span>
                  </span>
                  {item.selectedOptionObjs?.length > 0 && (
                    <span className="text-xs text-gray-500 dark:text-gray-400 font-medium mt-0.5 leading-tight pr-4 transition-colors">
                ...
                <div className="flex flex-col items-end shrink-0">
                  ...
                  <div className="flex items-center gap-3 mt-1.5 bg-gray-50 dark:bg-slate-800 rounded-full px-2 py-0.5 border border-gray-100 dark:border-slate-700 transition-colors">
                    <button
                      onClick={() => handleUpdateCartItemQty(id, item.qty - 1)}
                      className="text-gray-400 dark:text-gray-500 hover:text-orange-500 dark:hover:text-orange-400 font-black px-1 transition-colors"
                    >
                      -
                    </button>
                    <span className="text-[11px] font-bold text-gray-600 dark:text-gray-300 w-3 text-center transition-colors">
                      {item.qty}
                    </span>
                    <button
                      onClick={() => handleUpdateCartItemQty(id, item.qty + 1)}
          ...
          {Object.entries(cart.combos || {}).map(([id, combo]) => (
            <div
              key={id}
              className="mb-2 border-b border-gray-100 dark:border-slate-800 pb-4 bg-orange-50/50 dark:bg-orange-500/5 p-3 rounded-2xl transition-colors"
            >
              <div className="flex justify-between items-center font-bold text-sm text-orange-800 dark:text-orange-300 transition-colors">
                <span>
                  Combo {combo.name}{' '}
                  <span className="text-orange-600 dark:text-orange-400 ml-1">x{combo.qty}</span>
                </span>
          ...
          <div className="mt-6 pt-4 border-t border-gray-100 dark:border-slate-800 transition-colors">
            <div className="flex justify-between items-center mb-3">
              <h4 className="text-[11px] font-bold text-orange-800 dark:text-orange-300 uppercase tracking-widest flex items-center gap-2 transition-colors">
                <Sparkles size={14} className="fill-orange-500 text-orange-500" /> Thêm vào cho đủ
                vị?
              </h4>
              <button
            ...
            <div className="flex gap-3 overflow-x-auto no-scrollbar pb-2">
              {crossSellItems.map((item) => {
                ...
                return (
                  <div
                    key={item.id}
                    className="min-w-[130px] bg-gray-50 dark:bg-slate-800/80 p-3 rounded-2xl border border-gray-100 dark:border-slate-700 hover:border-orange-200 dark:hover:border-orange-500 transition-all flex flex-col justify-between"
                  >
                    <div>
                    ...
                    <button
                      disabled={!available}
                      onClick={() =>
                        handleAddToCart(item, getCartItemQty(item) + 1, false, hasOptions)
                      }
                      className="w-full py-2 bg-orange-500 text-white text-xs font-extrabold rounded-xl shadow-md shadow-orange-100 dark:shadow-orange-900/20 active:scale-95 transition-all disabled:bg-gray-300 disabled:text-gray-500 disabled:shadow-none"
                    >
