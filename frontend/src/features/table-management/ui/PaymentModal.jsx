...
import { paymentService } from '@features/payment/api/paymentService.js';
...
const PaymentModalContent = ({ isOpen, onClose, table, order, currentUser, onPaymentSuccess }) => {
        ...
        const items = order.orderItems
          ...
          .map((i) => ({
            ...
            selectedOptionValueIds:
              i.options?.map((opt) => opt.valueId).filter((id) => id != null) || [],
          }));
        const combos = order.orderItems
          .filter((i) => i.combo)
          .map((i) => ({ comboId: i.combo.id, quantity: i.quantity, notes: i.notes }));
  ...
  const handleCreatePayosQR = async () => {
    ...
    try {
      ...
      const nextPreview = {
        ...previewData,
        subtotalAmount:
          data.subtotalAmount ?? previewData?.subtotalAmount ?? getOrderSubtotalAmount(order),
        discountAmount:
          data.discountAmount ?? previewData?.discountAmount ?? getOrderDiscountAmount(order),
        finalAmount:
          data.finalAmount ?? data.amount ?? previewData?.finalAmount ?? getOrderFinalAmount(order),
  ...
  const subtotalAmount =
    ...
    0;
  const discountAmount =
    previewData?.discountAmount ?? payosData?.discountAmount ?? getOrderDiscountAmount(order) ?? 0;
  const finalAmount =
    previewData?.finalAmount ??
    payosData?.finalAmount ??
    payosData?.amount ??
              ...
              <div className="text-center space-y-4 bg-gray-50 p-6 rounded-2xl border-2 border-dashed border-blue-200 relative overflow-hidden">
                {payosStatus === 'expired' && (
                  <div className="absolute inset-0 bg-white/80 backdrop-blur-[2px] z-10 flex flex-col items-center justify-center p-4 text-center">
                    ...
                    <p className="text-xs text-gray-500 mb-4">
                      Vui lòng tạo lại mã mới để tiếp tục
                    </p>
                    <button
                      onClick={handleCreatePayosQR}
                ...
                <div className="space-y-1">
                  <div className="flex items-center justify-center gap-2 mb-1">
                    <div
                      className={`px-3 py-1 rounded-full text-xs font-bold flex items-center gap-1.5 ${timeLeft < 60 ? 'bg-red-100 text-red-600 animate-pulse' : 'bg-blue-100 text-blue-600'}`}
                    >
                      <Loader2
                        size={12}
                        className={payosStatus === 'waiting' ? 'animate-spin' : ''}
                      />
          ...
          <div className="bg-gray-50 p-5 rounded-2xl space-y-3 border">
            <div className="flex justify-between text-sm">
              ...
              <span className="font-semibold text-gray-700">
                {subtotalAmount.toLocaleString()}đ
              </span>
            </div>
            {discountAmount > 0 && (
              <div className="flex justify-between text-sm">
