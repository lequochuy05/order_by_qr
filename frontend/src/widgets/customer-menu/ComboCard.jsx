...
const ComboCard = ({ combo, onAddToCart, quantity = 0 }) => {
  ...
  return (
    <div
      className={`p-5 bg-gradient-to-br from-blue-500 via-blue-400 to-blue-500 text-white rounded-3xl shadow-[0_8px_20px_rgb(249,115,22,0.25)] relative overflow-hidden transition-all duration-300 active:scale-[0.98] h-full flex flex-col justify-between ${available ? '' : 'opacity-70'}`}
    >
      {/* Decorative flame */}
      ...
      <div className="relative z-10">
        <div className="flex items-center gap-2 mb-1">
          <span className="bg-white/20 backdrop-blur-md text-white text-[10px] px-2 py-0.5 rounded-full uppercase font-bold tracking-widest border border-white/20">
            COMBO
          </span>
          {!available && (
            <span className="bg-white text-red-600 text-[10px] px-2 py-0.5 rounded-full uppercase font-black">
              Hết hàng
            </span>
          )}
        </div>
