    ...
    >
      <div className="relative w-full pt-[100%] bg-gray-50 dark:bg-slate-800 overflow-hidden">
        ...
        {!available && (
          <div className="absolute inset-0 bg-black/45 flex items-center justify-center">
            <span className="px-3 py-1 rounded-full bg-white text-red-600 text-[10px] font-black uppercase">
              Hết hàng
            </span>
          </div>
        )}
      ...
      <div className="p-3 flex flex-col flex-1 justify-between transition-colors duration-500">
        ...
        <div className="mt-3 flex flex-col gap-2">
          ...
          <div
            className={`flex items-center rounded-full p-1 bg-orange-50 dark:bg-orange-500/10 transition-colors duration-500 ${item.itemOptions?.length > 0 ? 'justify-center w-[5.5rem] ml-auto' : 'justify-between'}`}
          >
            {item.itemOptions && item.itemOptions.length > 0 ? (
              ...
              <>
                ...
                <span
                  className={`text-xs font-bold text-orange-700 dark:text-orange-400 transition-colors ${quantity === 0 ? 'opacity-0' : ''}`}
                >
                  {quantity}
                </span>
