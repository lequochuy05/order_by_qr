import { X, ImageIcon, Plus, Trash2, ChevronDown, ChevronUp, Sparkles, Loader2 } from 'lucide-react';

const MenuItemModal = ({ isOpen, onClose, onSubmit, categories, formData, setFormData, preview, handleFileChange, isSubmitting, aiScanning }) => {
                ...
                <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex flex-col items-center justify-center gap-2">
                  ...
                  {preview && (
                    <button
                      ...
                      className="px-4 py-2 bg-gradient-to-r from-blue-500 to-indigo-500 text-white rounded-full text-xs font-black shadow-lg hover:scale-105 transition-transform flex items-center gap-1.5 disabled:opacity-60 disabled:cursor-not-allowed"
                    >
                      {aiScanning ? (
                        <><Loader2 size={14} className="animate-spin" /> Đang quét...</>
                      ) : (
                        <><Sparkles size={14} /> Magic Scan</>
                      )}
                    </button>
                  )}
                </div>
