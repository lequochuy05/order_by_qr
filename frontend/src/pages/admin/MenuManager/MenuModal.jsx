  ...
  return (
    <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4 backdrop-blur-sm animate-in fade-in duration-200">
      <div className="bg-white rounded-[2rem] w-full max-w-2xl shadow-2xl animate-in zoom-in duration-300 flex flex-col max-h-[90vh]">
        ...
        {/* Body - Scrollable */}
        <form id="menuForm" onSubmit={handleLocalSubmit} className="p-8 space-y-8 overflow-y-auto custom-scrollbar">

          {/* Thông tin cơ bản */}
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            <div className="space-y-5">
              <div>
                <label className="text-[10px] font-black text-gray-400 uppercase tracking-[0.2em] ml-1 mb-2 block">Tên món ăn <span className="text-red-500">*</span></label>
                <input
                  type="text" placeholder="Ví dụ: Trà sữa thái xanh"
                  className={`w-full px-5 py-3.5 bg-gray-50 border-2 rounded-2xl outline-none text-sm transition-all font-medium ${errors.name ? 'border-red-500 ring-red-50' : 'border-transparent focus:bg-white focus:ring-4 focus:ring-orange-500/10 focus:border-orange-500'
                    }`}
                  value={formData.name}
                  onChange={e => {
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="text-[10px] font-black text-gray-400 uppercase tracking-[0.2em] ml-1 mb-2 block">Giá bán <span className="text-red-500">*</span></label>
                  <div className="relative">
                    <input
                      type="number" placeholder="0"
                      className={`w-full pl-5 pr-10 py-3.5 bg-gray-50 border-2 rounded-2xl outline-none text-sm font-black transition-all ${errors.price ? 'border-red-500 ring-red-50' : 'border-transparent focus:bg-white focus:ring-4 focus:ring-orange-500/10 focus:border-orange-500 text-orange-600'
                        }`}
                    ...
                    />
                    <span className="absolute right-4 top-1/2 -translate-y-1/2 text-[10px] font-bold text-gray-400">đ</span>
                  </div>
                  {errors.price && (
                    <p className="text-red-500 text-[11px] mt-1.5 flex items-center gap-1.5 font-bold animate-in slide-in-from-top-1">
                ...
                <div>
                  <label className="text-[10px] font-black text-gray-400 uppercase tracking-[0.2em] ml-1 mb-2 block">Danh mục <span className="text-red-500">*</span></label>
                  <select
                    className={`w-full px-4 py-3.5 bg-gray-50 border-2 rounded-2xl outline-none text-sm font-bold appearance-none transition-all cursor-pointer ${errors.categoryId ? 'border-red-500' : 'border-transparent focus:bg-white focus:ring-4 focus:ring-orange-500/10 focus:border-orange-500'
                      }`}
            ...
            <div className="relative group">
              <label className="text-[10px] font-black text-gray-400 uppercase tracking-[0.2em] ml-1 mb-2 block">Hình ảnh đại diện</label>
              <div className={`border-2 border-dashed rounded-3xl p-2 text-center h-[145px] flex flex-col items-center justify-center relative overflow-hidden transition-all duration-300 ${preview ? 'border-orange-200 bg-orange-50/20' : 'border-gray-100 bg-gray-50/50 hover:border-orange-200'
                }`}>
                {preview ? (
                  <img src={preview} className="w-full h-full object-cover rounded-[1.25rem] shadow-sm" alt="Preview" />
                ) : (
                  <div className="space-y-2 text-gray-300 py-4">
                    <ImageIcon className="mx-auto" size={32} />
                    <p className="text-[9px] uppercase font-black tracking-tighter">Click để tải ảnh</p>
                  </div>
                )}
                ...
                <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-all duration-300 flex flex-col items-center justify-center gap-2 backdrop-blur-[2px]">
                  <label htmlFor="itemFile" className="px-5 py-2 bg-white text-gray-800 rounded-full text-[10px] font-black cursor-pointer shadow-xl hover:scale-105 active:scale-95 transition-all">
          ...
          <div className="space-y-5">
            <div className="flex justify-between items-center px-1">
              <h3 className="text-xs font-black text-gray-800 uppercase tracking-[0.2em] flex items-center gap-2.5">
                ...
                Tùy Chọn Mở Rộng
                <span className="px-2 py-0.5 bg-gray-100 text-gray-500 rounded-lg text-[9px] font-black">{formData.itemOptions.length}</span>
              </h3>
              <button
            ...
            <div className="space-y-6">
              {formData.itemOptions.map((opt, optIdx) => (
                <div key={optIdx} className="bg-gray-50/50 rounded-3xl p-6 border border-gray-100 space-y-6 relative group/option animate-in slide-in-from-bottom-2 duration-500">
                  <button
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-12 gap-4">
                    <div className="md:col-span-6">
                      <label className="text-[9px] font-bold text-gray-400 uppercase tracking-widest mb-1.5 block ml-1">Tên nhóm tùy chọn</label>
                      <input
                        type="text" placeholder="Ví dụ: Kích cỡ, Topping..."
                    ...
                    <div className="md:col-span-3">
                      <label className="text-[9px] font-bold text-gray-400 uppercase tracking-widest mb-1.5 block ml-1">Bắt buộc?</label>
                    ...
                    <div className="md:col-span-3">
                      <label className="text-[9px] font-bold text-gray-400 uppercase tracking-widest mb-1.5 block ml-1">Chọn tối đa</label>
                      <input
                      ...
                      {errors[`option_${optIdx}_max`] && (
                        <p className="text-red-500 text-[9px] mt-1 text-center font-bold">{errors[`option_${optIdx}_max`]}</p>
                      )}
                    </div>
                  </div>
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="grid grid-cols-1 gap-2.5">
                      {opt.optionValues.map((val, valIdx) => (
                        <div key={valIdx} className="grid grid-cols-12 gap-3 items-center animate-in fade-in slide-in-from-right-2 duration-300">
                          <div className="col-span-7">
                            <input
                              type="text" placeholder="Tên (Size L, Thêm đường...)"
                            ...
                            {errors[`option_${optIdx}_val_${valIdx}_name`] && (
                              <p className="text-red-500 text-[9px] mt-1 flex items-center gap-1 font-bold">
                                <AlertCircle size={10} /> {errors[`option_${optIdx}_val_${valIdx}_name`]}
                              </p>
                            )}
                          </div>
                          <div className="col-span-4">
                            <div className="relative">
                              ...
                              />
                              <span className="absolute right-3 top-1/2 -translate-y-1/2 text-[9px] text-gray-400 font-bold">đ</span>
                            </div>
                          </div>
                          <button
              ...
              {formData.itemOptions.length === 0 && (
                <div className="text-center py-12 bg-gray-50/50 rounded-[2rem] border-2 border-dashed border-gray-100">
                  <p className="text-xs text-gray-400 font-bold uppercase tracking-tighter opacity-60">Món này chưa có tùy chọn đi kèm</p>
                </div>
              )}
            </div>
          </div>
        ...
        <div className="px-8 py-6 border-t bg-gray-50/50 rounded-b-[2rem] flex gap-4 shrink-0">
          ...
          >
            {isSubmitting ? (
              ...
              formData.id ? 'Lưu thay đổi' : 'Tạo món ăn'
            )}
          </button>
        </div>
      </div>
