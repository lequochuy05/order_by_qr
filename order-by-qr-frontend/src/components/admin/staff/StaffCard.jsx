...
import { Mail, Phone, Shield, User, Pencil, Trash2, UtensilsCrossed } from 'lucide-react';
...
const StaffCard = ({ staff, onEdit, onDelete }) => {
  ...
  const getRoleConfig = () => {
    switch (staff.role) {
      case 'MANAGER':
        return { label: 'Quản lý', color: 'bg-purple-100 text-purple-600', icon: <Shield size={20} /> };
      case 'CHEF':
        return { label: 'Đầu bếp', color: 'bg-orange-100 text-orange-600', icon: <UtensilsCrossed size={20} /> };
      default:
        return { label: 'Nhân viên', color: 'bg-blue-100 text-blue-600', icon: <User size={20} /> };
    }
  };
  ...
  return (
    <div className="bg-white rounded-3xl p-5 shadow-sm border border-gray-100 hover:shadow-md transition-all flex flex-col h-full group">
      ...
      <div className="flex justify-between items-start mb-4">
        <div className={`p-2.5 rounded-2xl ${roleConfig.color}`}>
          {roleConfig.icon}
        </div>
      ...
      <div className="flex flex-col items-center mb-6">
        ...
        <span className="text-xs text-gray-500 font-medium">{roleConfig.label}</span>
      </div>
