...
const TableFormModal = ({ isOpen, onClose, initialData, onSubmit }) => {
    const [formData, setFormData] = useState({ id: null, tableNumber: '', capacity: 4, status: 'Trống', qrCodeUrl: '' });

    useEffect(() => {
        if (isOpen) {
            setFormData(initialData || { id: null, tableNumber: '', capacity: 4, status: 'Trống', qrCodeUrl: '' });
        }
    }, [isOpen, initialData]);
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
            <div className="bg-white rounded-2xl w-full max-w-md shadow-2xl overflow-hidden animate-in fade-in zoom-in duration-200">
                ...
                <div className="p-6 space-y-4">
                    <div>
                        ...
                        <input type="text" className="w-full p-2 border rounded-lg focus:ring-2 focus:ring-orange-500 outline-none"
                            ...
                            placeholder="VD: 101"
                            disabled={!!formData.id} // Thường không cho sửa số bàn để tránh lỗi logic
                        />
                    </div>
                    <div>
                ...
                <div className="px-6 py-4 border-t bg-gray-50 flex justify-end gap-2">
                    ...
                    <button onClick={handleSave} className="px-4 py-2 bg-orange-500 text-white font-bold hover:bg-orange-600 rounded-lg">Lưu</button>
                </div>
            </div>
        </div>
    );
